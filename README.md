...
- 🚫 **宣传/广告过滤**：命中关键词（拉群、加微信、提前领、二维码等）的动态自动跳过不推（可在 `blocklist.txt` 自定义）
...
## ⚡ 低延迟 / 多 UP 主：feed/all 模式

默认 `space` 模式逐个拉每个 UP 的空间动态：UP 越多、请求越多，频率不宜太高（建议 ≥3 分钟），
否则容易被风控 412。

如果你要监控**较多 UP**、还想要**1 分钟级低延迟**，用 `all` 模式：

- 改走 B站 `feed/all` 接口，**一次请求**就拿到「你关注的所有 UP」的最新动态，再按名单过滤。
- 每轮通常只有 **1 个请求**（无论监控多少 UP），请求量极低，1 分钟一次也很安全。
- **前提**：用这个监控账号**关注**名单里的所有 UP（`feed/all` 只返回你关注的人的动态）。
- 开启：在 `.env` 里设 `BILI_FEED_MODE=all`。关注的人特别多时，可调大 `FEED_ALL_MAX_PAGES`。

> 建议用一个**专门的小号**只关注这些 UP —— 这样 `feed/all` 第一页永远是这些人的动态、绝不漏，
> 也不会把它们混进你主号的首页推荐。
...
很多 UP 主会发「加群/加微信/提前领/扫码」之类的引流广告。本工具会**过滤掉正文命中宣传关键词的动态**
（跳过不推、静默标记已读），只把真正的内容推给你。

- 词表在 [`blocklist.txt`](blocklist.txt)，每行一个词、`#` 注释，匹配不区分大小写。
- 想多屏蔽就加词，想放行某类就删词；改完下次运行即生效（没有该文件时用内置默认词）。
- 注意：只能匹配**动态正文**。如果广告内容只在图片（二维码海报）里、正文却很正常，关键词就拦不住——
  这类需要 OCR/图像识别，本工具暂不处理。
